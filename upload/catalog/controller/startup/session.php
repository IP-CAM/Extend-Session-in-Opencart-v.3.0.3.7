...
class ControllerStartupSession extends Controller {
	public function index() {
		if (isset($this->request->get['api_token']) && isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/') {
			...
			setcookie($this->config->get('session_name'), $this->session->getId(), time() + $this->config->get('session_lifetime'), ini_get('session.cookie_path'), ini_get('session.cookie_domain'));
		}
	}
}